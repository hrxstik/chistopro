...
import { Checklist, ChecklistTask, TaskStatus } from '@/types/checklist';
    ...
    try {
      ...
      if (daysSinceLastVisit > 0 && daysSinceLastVisit <= MAX_MISSED_DAYS && lastChecklist) {
        ...
        for (let i = 1; i <= daysToGenerate; i++) {
          ...
          const missedDateString = `${missedDate.getFullYear()}-${String(missedDate.getMonth() + 1).padStart(2, '0')}-${String(missedDate.getDate()).padStart(2, '0')}`;
          ...
          if (!existingChecklist) {
            ...
            if (selectedRooms.length > 0) {
              missedChecklist = generateChecklist(rooms, missedDateString);
            } else {
              missedChecklist = {
      ...
      if (lastChecklist && (now - lastChecklist.createdAt) < TWENTY_FOUR_HOURS) {
        ...
        if (daysSinceLastVisit > MAX_MISSED_DAYS) {
          ...
          console.log(`User hasn't visited for ${daysSinceLastVisit} days (more than ${MAX_MISSED_DAYS}), creating new checklist as reset point`);
          ...
          const selectedRooms = rooms.filter(r => r.checked);
          if (selectedRooms.length > 0) {
            checklist = generateChecklist(rooms, today);
            await checklistStorage.saveChecklist(checklist);
            console.log(`New checklist generated after ${daysSinceLastVisit} days absence - new reset point`);
          } else {
